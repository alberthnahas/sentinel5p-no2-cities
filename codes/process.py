######################################################################################
# Purpose:      Acquire, process, and analyze Sentinel-5P Level 2 total column
#               Nitrogen Dioxide (NO₂) data for multiple urban areas.
#
# Description:  This script connects to the openEO platform (Copernicus Dataspace)
#               to download daily aggregated total column NO₂ from Sentinel-5P for
#               specified cities. It then generates and visualizes:
#                 - Time series plots showing daily NO₂ levels alongside a 7-day
#                   rolling mean. Missing data points (NaNs) will appear as gaps.
#                 - Boxplots comparing NO₂ levels on weekdays versus weekends.
#                 - Boxplots showing seasonal variations in NO₂ levels.
#               The script also includes functionality to measure and report its
#               execution time for different sections. This version does not
#               perform data imputation.
#
# Input Files:  - None directly for initial run if RUN_OPENEO_DATA_ACQUISITION is True.
#               - Requires NO2_YYYY_{city}.nc files (e.g., "NO2_2024_Jakarta.nc")
#                 (generated by Section A of this script or a previous run) in the
#                 local directory if RUN_OPENEO_DATA_ACQUISITION is False for
#                 Sections B-E to process data.
#
# Output Files: - NetCDF: NO2_YYYY_{city}.nc (e.g., "NO2_2024_Jakarta.nc") - one per city,
#                           containing daily mean NO₂ values (Generated by Section A).
#               - PNG: "no2_timeseries_YYYY.png" (Time series plots for all cities)
#               - PNG: "no2_daytype_YYYY.png" (Boxplot comparing weekday/weekend levels)
#               - PNG: "no2_seasonal_YYYY.png" (Boxplot comparing seasonal levels)
#
# Key Libraries/
# Modules Used: - openeo (for data access from Copernicus Dataspace)
#               - xarray (for handling NetCDF data)
#               - pandas (for data manipulation, time series analysis)
#               - numpy (for numerical operations)
#               - matplotlib & seaborn (for plotting)
#               - time (for script execution timing)
#
# Author:       Alberth Nahas (alberth.nahas@bmkg.go.id)
# Created Date: 2025-05-09
# Version:      1.0.1 (Fixed authentication issue for openEO connection)
#
# Disclaimer:   This script is provided for educational and operational use to analyze
#               Sentinel-5P NO₂ data. Results are dependent on the availability and
#               quality of the satellite data and the performance of the openEO backend.
#               Ensure correct configuration for openEO connection and target year/cities.
#               This version does not perform data imputation for gaps; missing data
#               will be represented as NaNs or gaps in plots.
######################################################################################
...
aois = {
    ...
    "New_Delhi": {"type": "Point", "coordinates": [77.21, 28.61]},
